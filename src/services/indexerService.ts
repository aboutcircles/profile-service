...
export class IndexerService {
  private circlesDataForSubscriptions: any;
  private circlesDataForFetch: any;
  ...
  async initialize(): Promise<void> {
    ...
    const circlesRpcForSubscription = new CirclesRpc(config.wsEndpoint || config.rpcEndpoint);
    this.circlesDataForSubscriptions = new CirclesData(circlesRpcForSubscription);

    const circlesRpcForFetch = new CirclesRpc(config.rpcEndpoint);
    this.circlesDataForFetch = new CirclesData(circlesRpcForFetch);
  ...
  private async bufferLiveEvents(): Promise<void> {
    const events = await this.circlesDataForSubscriptions.subscribeToEvents();
  ...
  private async catchUpOnMissedEvents(fromBlock: number, toBlock: number) {
    ...
    const tryFetch = async (attempt: number): Promise<void> => {
      ...
      try {
        const events = await this.circlesDataForFetch.getEvents(
          null,
          fromBlock + 1,
          toBlock,
          [
